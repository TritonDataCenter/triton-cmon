...
function _fetchVm(log, vmapi, arg, lobj, datacb, cb) {
    ...
    vmapi.listVms(vmapiFilter, function _handleVmapiResponse(err, obj_arr) {
        ...
        mod_vasync.forEachPipeline({
            'inputs': obj_arr,
            'func': function _mapObjToVm(obj, next) {
                /*
                 * Bootstrap should only concern itself with actively running
                 * VMs. All other state transitions will be dealt with by
                 * changefeed processing.
                 */
                if (obj && obj.state && obj.state === 'running') {
                    lib_common.mapVm(
                        obj,
                        'Bootstrapper',
                        function _pushVm(mErr, vm) {
                            /*
                             * Empirical evidence suggests that VMAPI can
                             * respond with VM objects populated with data that
                             * cannot be reasonably handled by CMON. Rather than
                             * explode when this happens we skip over the VM and
                             * log error to be helpful to operators and
                             * postmortem debugging.
                             */
                            if (mErr) {
                                log.warn({ err: mErr, vm: obj },
                                    'Skipping incomplete vm object');
                            } else {
                                log.trace({ vmAdd: vm }, 'Push bootstrap vm');
                                vms.push(vm);
                            }
                    ...
                    log.trace({ skippedVm: obj }, 'Skipping vm due to state');
                    next();
                }
