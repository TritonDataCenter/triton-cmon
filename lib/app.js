...
var BASE_TLS_CERT = '/data/tls/base_cert.pem';
var BASE_TLS_KEY = '/data/tls/base_key.pem';
var WILDCARD_TLS_CERT = '/data/tls/wildcard_cert.pem';
var WILDCARD_TLS_KEY = '/data/tls/wildcard_key.pem';

var SNIContexts = {
    base: {
        cert: mod_fs.readFileSync(BASE_TLS_CERT),
        key: mod_fs.readFileSync(BASE_TLS_KEY)
    },
    wildcard: {
    ...
    var serverOpts = {
        httpsServerOptions: {
            ...
            SNICallback: function _handleSNI(servername, cb) {
                /* If the servername starts with cmon, we want the base cert */
                if (servername.match(/^cmon/i)) {
                    cb(mod_tls.createSecureContext(SNIContexts.base));
                    return;
                }

                /* If we're not the base, we use the wildcard cert */
