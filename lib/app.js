...
var ROOT_TLS_CERT = '/data/tls/root_cert.pem';
var ROOT_TLS_KEY = '/data/tls/root_key.pem';
var WILDCARD_TLS_CERT = '/data/tls/wildcard_cert.pem';
var WILDCARD_TLS_KEY = '/data/tls/wildcard_key.pem';

var SNIContexts = {
    root: {
        cert: mod_fs.readFileSync(ROOT_TLS_CERT),
        key: mod_fs.readFileSync(ROOT_TLS_KEY)
    },
    wildcard: {
        cert: mod_fs.readFileSync(WILDCARD_TLS_CERT),
        key: mod_fs.readFileSync(WILDCARD_TLS_KEY)
    }
};
...
function App(opts) {
    ...
    var serverOpts = {
        httpServerOptions: {
            ...
            certificate: mod_fs.readFileSync(ROOT_TLS_CERT),
            key: mod_fs.readFileSync(ROOT_TLS_KEY),
            requestCert: true,
            rejectUnauthorized: false,
            SNICallback: function _handleSNI(servername, cb) {
                /* If the servername starts with cmon, we want the root cert */
                if (servername.match(/^cmon/i)) {
                    cb(mod_tls.createSecureContext(SNIContexts.root));
                    return;
                }

                /* If we're not the root, we use the wildcard cert */
                cb(mod_tls.createSecureContext(SNIContexts.wildcard));
            }
        }
    };
