...
function App(opts) {
    ...
    if (process.env.BOOTSTRAP_ONLY) {
        updater.on('finish', function _updaterFinished() {
            // all done, time to kill everything
            self.vmapi.close();
            self.cnapi.close();
            self.mahi.close();

            /* I had to add a hack to cfListener:
             *
             *       var client = mod_restify.createClient(clientOpts);
             *   +   self.crapClient = client;
             *
             * because otherwise cfListener never closes that socket it creates
             * itself. Eww. It really should be modified to let you pass a client
             * in instead.
             */
            cfListener.crapClient.close();

            self.log.info('BOOTSTRAP_ONLY specified and bootstrap complete, ' +
                'exiting.');
        });

        // We don't want backoff starting bootstrap again when we're done
        cfListener.backoff_opts = null;
    }
    ...
    function _processCNs() {
        ...
        if (process.env.BOOTSTRAP_ONLY) {
            cnapiPoller.on('end', function () {
                cnapiPoller.unpipe(updater);
            });
        } else {
    ...
    cfListener.on('bootstrap', function _bootstrapBegin() {
        ...
        if (process.env.BOOTSTRAP_ONLY) {
            vmBootstrapper.pipe(updater, { end: true });
        } else {
...
App.prototype.start = function start(cb) {
    ...
    if (process.env.BOOTSTRAP_ONLY) {
        self.log.info('not listening, BOOTSTRAP_ONLY specified');
        cb();
        return;
    }
